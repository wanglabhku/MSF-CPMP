...
if __name__ == '__main__':
    for i in range(1, 11):
        ...
        for epoch in range(epochs):
            ...
            train_epoch_acc = corrent / total
            train_epoch_loss = loss_avg / step
            ...
            t1, t2 = pd.DataFrame(test_list1, columns=['predict']), \
                pd.DataFrame(y_true_test[:688], columns=['true'])
            tt = pd.concat([t1, t2], axis=1)

            os.makedirs(f'./pred_data/cnn/test1/{i}/', exist_ok=True)

            pd.DataFrame(tt).to_csv('./pred_data/cnn/test1/{}/experiment_{}_predicted_values.csv'.format(i, epoch+1), index=False)

            t1, t2 = pd.DataFrame(test_list2, columns=['predict']), \
                pd.DataFrame(y_true_test[:688], columns=['true'])
            tt = pd.concat([t1, t2], axis=1)

            os.makedirs(f'./pred_data/cnn/test2/{i}/', exist_ok=True)

            pd.DataFrame(tt).to_csv('./pred_data/cnn/test2/{}/experiment_{}_predicted_values.csv'.format(i, epoch+1),
                                    index=False)
