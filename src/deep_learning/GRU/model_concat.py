...
from GRU_model import GRU_Classifier
import pandas as pd
from GRU_model import create_dataset_seq,create_dataset_number,d_loadar
input_size = 128
hidden_size = 32
num_layers = 5
output_size = 1
...
if __name__ == '__main__':
    for i in range(1,11):
        ...
        model = GRU_Classifier(input_size, hidden_size, output_size,num_layers, ).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            t1,t2 = pd.DataFrame(test_list1,columns=['predict']),pd.DataFrame(y_true_test['Permeability'].values[:688],columns=['true'])
            tt = pd.concat([t1,t2],axis=1)
            os.makedirs(f'./pred_data/GRU/test1/{i}/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/GRU/test1/{}/experiment_{}_predicted_values.csv'.format(i,epoch+1),index=False)

            t1, t2 = pd.DataFrame(test_list2, columns=['predict']), pd.DataFrame(y_true_test['Permeability'].values[:688],
                                                                                 columns=['true'])
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data/GRU/test2/{i}/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/GRU/test2/{}/experiment_{}_predicted_values.csv'.format(i, epoch+1),
                                    index=False)
