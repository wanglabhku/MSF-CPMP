...
from RNN_model import RNN_Classifier
import pandas as pd
from RNN_model import create_dataset_seq,create_dataset_number,d_loadar
input_size = 128
hidden_size = 32
num_layers = 2
output_size = 1
...
if __name__ == '__main__':
    for i in range(1,11):
        ...
        model = RNN_Classifier(input_size, hidden_size, output_size, num_layers ).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            os.makedirs(f'./pred_data/RNN/test1/{i}/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/RNN/test1/{}/experiment_{}_predicted_values.csv'.format(i, epoch + 1),
                                    index=False)
            ...
            t1, t2 = pd.DataFrame(test_list2, columns=['predict']), pd.DataFrame(
                ...
                columns=['true'])  # 自己加的
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data/RNN/test2/{i}/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/RNN/test2/{}/experiment_{}_predicted_values.csv'.format(i, epoch + 1),
                                    index=False)
