...
def main_():
    for num in range(1, 11):
        ...
        for epoch in range(1, 51):
            ...
            plt.imshow(attention[8].detach().cpu().numpy(), cmap='viridis')
            ...
            os.makedirs(f'./pred_data_origin/transformer/{num}/test1/', exist_ok=True)
            pd.DataFrame(tt).to_csv(
                './pred_data_origin/transformer/{}/test1/experiment_{}_predicted_test_values.csv'.format(num, epoch),
                index=False)
            ...
            t1, t2 = pd.DataFrame(test_list2, columns=['predict']), pd.DataFrame(
                ...
                columns=['true'])
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data_origin/transformer/{num}/test2/', exist_ok=True)
            pd.DataFrame(tt).to_csv(
                './pred_data_origin/transformer/{}/test2/experiment_{}_predicted_test_values.csv'.format(num, epoch),
                index=False)
            ...
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data_origin/transformer/{num}/val/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data_origin/transformer/{}/val/experiment_{}_predicted_valid_values.csv'.format(num, epoch),index=False)
