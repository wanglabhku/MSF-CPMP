...
def main():
    for num in range(1, 11):
        ...
        gcn_net = GATPredictor(in_feats=n_feats,
                               hidden_feats=[60, 20],
                               n_tasks=2,
        ...
        for epoch in range(1, 201):
            ...
            os.makedirs(f'./pred_data/GAT/{num}/test1/', exist_ok=True)
            pd.DataFrame(tt).to_csv(
                './pred_data/GAT/{}/test1/experiment_{}_predicted_test_values.csv'.format(num, epoch),
                index=False)
            ...
            t1, t2 = pd.DataFrame(test_list2, columns=['predict']), pd.DataFrame(
                ...
                columns=['true'])
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data/GAT/{num}/test2/', exist_ok=True)
            pd.DataFrame(tt).to_csv(
                './pred_data/GAT/{}/test2/experiment_{}_predicted_test_values.csv'.format(num, epoch),
                index=False)
            ...
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data/GAT/{num}/val/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/GAT/{}/val/experiment_{}_predicted_valid_values.csv'.format(num, epoch),
                                    index=False)
