...
if __name__ == '__main__':
    fd_t = open("./pred_data/rf/result_test.txt", "a+")
    for i in tqdm(range(1,11)):
        ...
        os.makedirs(f'./pred_data/rf/test1/', exist_ok=True)

        pd.DataFrame(tt).to_csv('./pred_data/rf/test1/experiment_{}_predicted_values.csv'.format(i))

        t1, t2 = pd.DataFrame(mlist2, columns=['predict']), pd.DataFrame(test_y[:numtest], columns=['true'])
        tt = pd.concat([t1, t2], axis=1)

        os.makedirs(f'./pred_data/rf/test2/', exist_ok=True)

        pd.DataFrame(tt).to_csv('./pred_data/rf/test2/experiment_{}_predicted_values.csv'.format(i))

        os.makedirs(f'./pred_model/rf_model/', exist_ok=True)
        with open('./pred_model/rf_model/{}_rf_model_rbf.pkl'.format(i), 'wb') as f:
            pickle.dump(clf, f)
